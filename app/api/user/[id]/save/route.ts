...
export const PATCH = async(req: Request, { params }: { params: { id: string} }) =>{
    const id = params.id;
    const { postId, title, banner, category, text, creator } = await req.json();
    try {
        const user = await User.findById(params.id);

        if(!user) return new Response(JSON.stringify({ message: "Usuário não encontrado" }), { status: 404 });

        if(!postId || !title || !banner || !category || !text || !creator) {
        ...
        { $push: { saved: { postId, title, text, banner, category, creator, createdAt: new Date() } } });
